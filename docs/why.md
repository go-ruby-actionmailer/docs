...
`go-ruby-actionmailer/actionmailer` reimplements Ruby's `actionmailer` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-actionmailer/actionmailer` directly, with no Ruby runtime;
...
Because `actionmailer` is pure Go with cgo disabled, it also compiles to **WebAssembly**
